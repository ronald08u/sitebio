...
const groupsData = {
    ...
    actividad: {
        ...
        doctorAvatar: 'RT',
        gradient: 'linear-gradient(135deg, #2A9D8F, #457B9D)'
    },
    habitos: {
    ...
    rehabilitacion: {
        ...
        doctorAvatar: 'MR',
        gradient: 'linear-gradient(135deg, #E63946, #C5303A)'
    },
    hipertension: {
...
async function sendChatMessage() {
    ...
    if (message) {
        ...
        try {
            ...
            typingDiv.remove();
            addChatMessage('Lo siento, hubo un error al procesar tu consulta. Por favor, intenta de nuevo.', false);
        }
    }
}

async function getGeminiResponse(userMessage) {
    const systemPrompt = `Eres un asistente especializado en salud cardiovascular llamado CardioAsistente. Tu rol es:
...
function addChatMessage(message, isUser) {
    ...
    if (isUser) {
        ...
        messageDiv.innerHTML = `
            <div class="message-avatar" style="background: linear-gradient(135deg, var(--primary-red), var(--accent-coral)); border-radius: 50%; display: flex; align-items: center; justify-content: center; color: white; font-weight: 600;">${initials}</div>
